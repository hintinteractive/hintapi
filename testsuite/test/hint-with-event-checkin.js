...
//var _u_to_social_id = 'Facebook:100004954867265';
var _u1_name = 'user2';
//var _u_from_social_id = 'Facebook:100007978092636';
var _u2_name = 'user1';
...
describe('PATCH /api/event',function() {
	it('patch name',function (done) {
		...
		.send({
				...
				.end(function (err, res) {
					...
					res.body.should.have.deep.property('result.status');
					res.body.should.have.deep.property('result.flirt_options');
					res.body.should.have.deep.property('result.owners').and.be.instanceof(Array);
					res.body.should.have.deep.property('result.venue');
	...
	it('patch venue',function (done) {
		...
		.send({
					social_id : _event_id,
					venue : {
						social_id : 'Facebook:283980645075903',
						name : 'Western Grill',
						address : 'Western Grill',
						lat : 20.20,
		 				lng : 11.11,
						category: {
            				image: "test"
	...
	it('patch time',function (done) {
		if (!commonObj['auth_token']) done("Not Authenticated");
		
		api.patch('/api/event')
		.send({
					social_id : _event_id,
					start : _start_time + 2000,
					expiry : _expiry_time + 2000	
				})
				.set('Content-Type', 'application/json')
				.set('Accept', 'application/json')
				.set('X-ZUMO-AUTH', commonObj['auth_token'])
	...
	it('patch flirts options',function (done) {
		if (!commonObj['auth_token']) done("Not Authenticated");
	...
	it('get friend list',function (done) {
		...
		.query({
				...
				.end(function (err, res) {
					...
					res.body.should.have.property('result').and.be.an.instanceof(Array);
					for(var i=0;i<res.body.result.length;i++){
						if(res.body.result[i].social_id == _u2_social_id){
							res.body.result[i].should.have.deep.property('social_id').and.have.string('Facebook:');
							res.body.result[i].should.have.deep.property('name');
							res.body.result[i].should.have.deep.property('image');	
						
							if (config.verbose) console.log("GET /api/event/friend :".underline.green, JSON.stringify(res.body.result[i]));
	             			done();
						}	
					}    	
			 });
			...
			.query({
		       ...
		       .end(function (err, res) {
		    	   if (err) return done(err);
		    	    ...
		    	    if (config.verbose) console.log("GET GET /api/event for user2:".underline.green, JSON.stringify(res.body));
		    	    done();
				});	
	});
});	
